...
private:
    ...
    // bool islisten_ = false;              // listenfd 取值为 true, 客户端连上上来的 fd 取值 为false
    std::function<void()> readcallback_; // fd_ 为读事件的回调函数
...
public:
    ...
    void onmessage();                                // 处理对端发送过来的数据
    void setreadcallback(std::function<void()>);     // 设置 fd_ 读事件的回调函数
};
